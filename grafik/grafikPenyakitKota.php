...
<br>
<a href="grafikObatProv.php" class="btn btn-primary btn-lg active" role="button" aria-pressed="true" style="background-color: #297aa8 ; border-color: #297aa8; font-size: 11pt; " autocomplete="on">Infografi Obat</a>
<a href="grafikPenyakitProv.php" class="btn btn-primary btn-lg active" role="button" aria-pressed="true" style="background-color: #DC143C ; border-color: #DC143C; font-size: 11pt; " autocomplete="on">Kembali</a>
...
<!--https://obattradisionaltekanandarahtinggi.wordpress.com/2013/05/30/obat-tradisional-tekanan-darah-tinggi/ -->
	...
	<script>
		...
		var myChart = new Chart(ctx, {
			<?php 
				include 'config.php';
				$result = mysqli_query($conn, "SELECT * FROM kota WHERE id_prov = $provinsi");
				$i = mysqli_num_rows($result);
				$provinsi = mysqli_fetch_assoc($result);
				$label = "";
				foreach ($result as $prov) {
					$id =$prov["id_kota"];
					$hitung = mysqli_query($conn, "SELECT * FROM rekap_medis WHERE penyakit = '$penyakit' AND id_pasien IN(SELECT id_pasien FROM pasien WHERE id_kota = $id)");
					$jumlah = mysqli_num_rows($hitung);
					$data.=$jumlah;
					$label.='"';
					$label.=$prov["nama_kota"];
					$label.='"';
					if($i>1){
						$label.=',';
						$data.=',';

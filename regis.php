...
function registrasi(){
	...
	if( mysqli_fetch_assoc($result)){
		echo "<script>
			  ...
			  header("Location: daftar.php?error=pasien");
			  return false;
	}
	$result = mysqli_query($conn, "SELECT * FROM administrator WHERE email_A = '$email_P'");

	if( mysqli_fetch_assoc($result)){
		echo "<script>
				alert('username sudah terdaftar!')
			  </script>";
			  header("Location: daftar.php?error=admin");
			  return false;
	}
	...
	if( $password_P !== $password2) {
		...
		header("Location: daftar.php?error=pass");
		return false;
	}
	// enkripsi password
...
mysqli_query($conn, "INSERT INTO pasien VALUES ('', '$nama_P', '$password_P', '-', '$NIK_P', '$email_P', '$alamat_P', '$no_hpP', '$Goldar', '$kelamin_P', '$status', '$pekerjaan', '$ttl_P', $id_kota)");
			...
			$_SESSION["email_pasien"] = $email_P;
			$_SESSION["nama_pasien"] = $nama_P;
